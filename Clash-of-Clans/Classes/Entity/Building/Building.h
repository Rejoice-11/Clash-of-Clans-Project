...
class Building : public GameObject 
{
    public:
        ...
        void setGridPosition(const Vec2 & pos)
        {
            _gridPos = pos; 
        }
        ...
        { 
            return static_cast<int>(_data.type); 
        }
    
        // 获取建筑类型在此等级最大可建造数
    	int getBuildingMaxCount() const 
        { 
            return _data.maxBuildCount[_currentLevel];
        }
        ...
        // 资源消耗（建造/升级）
        int getBuildCostGold() const 
        { 
            return _data.goldCost[_currentLevel];
        }

        int getBuildCostElixir() const
        { 
            return _data.elixirCost[_currentLevel];
        }
        ...
        // 是否为防御建筑（影响巨人AI）
        virtual bool isDefenseStructure() const 
        {
            return false; 
        }
